...
class GMOptimizer():
    ...
    # value is numpy here
    def gm_register(self, name, value, model_name, hyperpara_list, gm_num,
                    gm_lambda_ratio_value, uptfreq):
        ...
        if np.ndim(value) >= 2:
            ...
            if 'mlp' in model_name:
                ...
                if "weight_ih" in name or "weight_hh" in name:
                    ...
                    print('gm_register base lstm ih and hh shape: ',
                          value.shape)
                ...
                if 'conv' in name or 'downsample' in name:
                    ...
                    print("gm_register base cnn conv downsample value shape: ",
                          value.shape)
                    base = value.shape[0] * value.shape[2] * value.shape[
                        3] / 20.0
                else:  # fc1
            ...
            if gm_lambda_ratio_value >= 0.0:
                ...
                reg_lambda = np.arange(1.0, reg_lambda_range,
                                       reg_lambda_range / gm_num)
            print("pi: ", pi)
            print("reg_lambda: ", reg_lambda)
            self.gmregularizers[name] = GMRegularizer(hyperpara=layer_hyperpara,
                                                      gm_num=gm_num,
                                                      pi=pi,
                                                      reg_lambda=reg_lambda,
                                                      uptfreq=uptfreq)

    def apply_GM_regularizer_constraint(self, labelnum, trainnum, epoch,
                                        weight_decay, f, name, step):
        ...
        if np.ndim(f.data.cpu().numpy()) < 2:
            ...
            self.gmregularizers[name].apply(labelnum, trainnum, epoch, f, name,
                                            step)
...
class GMRegularizer():
    ...
    def __init__(self,
                 ...
                 reg_lambda=None,
                 uptfreq=None):
        self.a, self.b, self.alpha, self.gm_num = hyperpara[0], hyperpara[
            1], hyperpara[2], gm_num
        print("init self.a, self.b, self.alpha, self.gm_num: ", self.a, self.b,
              self.alpha, self.gm_num)
        self.pi, self.reg_lambda = np.reshape(np.array(pi),
                                              (1, gm_num)), np.reshape(
                                                  np.array(reg_lambda),
                                                  (1, gm_num))
        ...
        print("init self.gmuptfreq, self.paramuptfreq: ", self.gmuptfreq,
              self.paramuptfreq)

    # calc the resposibilities for pj(wi)
    def calcResponsibility(self):
        # responsibility normalized with pi
        responsibility = gaussian.pdf(
            self.w_array,
            loc=np.zeros(shape=(1, self.gm_num)),
        ...
        self.responsibility = responsibility / (np.sum(
            responsibility, axis=1).reshape(self.w_array.shape))
        # print ("np.sum(self.responsibility, axis=1): ", np.sum(self.responsibility, axis=1))

    def update_GM_Prior_EM(self, name, step):
        # update pi
        self.reg_lambda = (2 * (self.a - 1) + np.sum(
            self.responsibility, axis=0)) / (2 * self.b + np.sum(
                self.responsibility * np.square(self.w_array), axis=0))
        if step % self.gmuptfreq == 0:
            print("name: ", name)
            print("np.sum(self.responsibility, axis=0): ",
                  np.sum(self.responsibility, axis=0))
            print(
            ...
            print(
                ...
                np.sum(self.responsibility, axis=0))
        # update reg_lambda
        self.pi = (np.sum(self.responsibility, axis=0) + self.alpha -
                   1) / (self.w_array.shape[0] + self.gm_num * (self.alpha - 1))
    ...
    def apply(self, labelnum, trainnum, epoch, f, name, step):
        if "_first_gate" in name:
            ...
            self.w_array = w_array_chunk[0].reshape(
                (-1, 1))  # used for EM update also
        elif "_second_gate" in name:
            w_array_chunk = self.chunk_array(f.data.cpu().numpy(), 4, 0)
            self.w_array = w_array_chunk[1].reshape(
                (-1, 1))  # used for EM update also
        elif "_third_gate" in name:
            w_array_chunk = self.chunk_array(f.data.cpu().numpy(), 4, 0)
            self.w_array = w_array_chunk[2].reshape(
                (-1, 1))  # used for EM update also
        elif "_fourth_gate" in name:
            w_array_chunk = self.chunk_array(f.data.cpu().numpy(), 4, 0)
            self.w_array = w_array_chunk[3].reshape(
                (-1, 1))  # used for EM update also
        else:
            # print ("name: ", name)
            # print ("self.w_array not first to fourth gate")
            self.w_array = f.data.cpu().numpy().reshape(
                (-1, 1))  # used for EM update also
        if epoch < 2 or step % self.paramuptfreq == 0:
            ...
            if "_first_gate" in name:
                ...
                self.reg_grad_w[0 * base:1 * base] = (
                    np.sum(self.responsibility * self.reg_lambda,
                           axis=1).reshape(self.w_array.shape) *
                    self.w_array).reshape(base, -1)
            elif "_second_gate" in name:
                self.reg_grad_w = np.zeros(f.grad.data.cpu().numpy().shape)
                base = int(self.reg_grad_w.shape[0] / 4)
                self.reg_grad_w[1 * base:2 * base] = (
                    np.sum(self.responsibility * self.reg_lambda,
                           axis=1).reshape(self.w_array.shape) *
                    self.w_array).reshape(base, -1)
            elif "_third_gate" in name:
                ...
                self.reg_grad_w = np.sum(self.responsibility * self.reg_lambda,
                                         axis=1).reshape(
        ...
        reg_grad_w_dev = (torch.from_numpy(
            (self.reg_grad_w.reshape(f.data.cpu().numpy().shape)) /
            float(normalization_coefficient))).float()
        ...
        if torch.cuda.is_available():
            f.grad.data.add_(1.0, reg_grad_w_dev.cuda())
        else:
